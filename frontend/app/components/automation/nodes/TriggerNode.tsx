...
export function TriggerNode({ data, id }: NodeProps<TriggerNodeData>) {
  ...
  return (
    <div 
      className={`
        min-w-[200px] rounded-lg border-2 p-4 shadow-lg cursor-pointer hover:border-green-400 transition-colors
        ${isConfigured 
          ? 'bg-neutral-900 border-green-500/50' 
    ...
    >
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Zap className={`w-4 h-4 ${isConfigured ? 'text-green-500' : 'text-neutral-400'}`} />
