...
interface ConditionNodeData {
  ...
  onConfigure?: (nodeData: ConditionNodeData, nodeId: string) => void;
...
export function ConditionNode({ data, id }: NodeProps<ConditionNodeData>) {
  ...
  return (
    <div 
      className={`
        min-w-[200px] rounded-lg border-2 p-4 shadow-lg cursor-pointer hover:border-blue-400 transition-colors
        ${isConfigured 
    ...
    >
      ...
      <Handle
        ...
        style={{
          background: isConfigured ? '#3b82f6' : '#6b7280',
          border: 'none',
          width: 8,
          height: 8,
        }}
      ...
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2">
          <Filter className={`w-4 h-4 ${isConfigured ? 'text-blue-500' : 'text-neutral-400'}`} />
      ...
      <div className="flex items-center gap-2">
        <div 
          className={`
            ...
            ${isConfigured ? 'bg-blue-500' : 'bg-yellow-500'}
          `}
        />
